...
class RefreshAll {
  init (resourceConfig, env) {
    this.logger = env.bootedServices.logger.child('stateResource:refreshAll')
  ...
  async run (event, context) {
    for (const key of Object.keys(this.rankings)) {
      ...
      if (value.source && value.factors && this.registry.registry[key]) {
        ...
        const riskScoreTrailModel = this.models[`${camelCase(value.namespace)}_${value.riskScoreTrailModel}`]
